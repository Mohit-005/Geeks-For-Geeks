...
class Solution{
    ...
    if(node==null)
        return null;
    HashMap<Integer,Node> uwu=new HashMap<Integer,Node>();
    return help(node,uwu);   
    }
    Node help(Node node,HashMap<Integer,Node> uwu)
    {
        if(uwu.containsKey(node.val))
        return uwu.get(node.val);
        Node clone = new Node(node.val);
        uwu.put(node.val,clone);
        for(Node nbrs:node.neighbors)
        {
            clone.neighbors.add(help(nbrs,uwu));
        }
        return clone;
    }
}
